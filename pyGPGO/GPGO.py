...
class GPGO:
    def __init__(self, surrogate, acquisition, f, parameter_dict, n_jobs=1):
        ...
        # print('self.parameter_range:', self.parameter_range)
        self.history = []
    ...
    def _sampleParam(self):
        ...
        for index, param in enumerate(self.parameter_key):
            if self.parameter_type[index] == 'int':
                ...
                d[param] = round(np.random.uniform(self.parameter_range[index][0], self.parameter_range[index][1]), 2)
            else:
    ...
    def _optimizeAcq(self, method='L-BFGS-B', n_start=100):
        ...
        while flag:
            ...
            start_points_dict = [self._sampleParam() for i in range(n_start)]
            start_points_arr = np.array([list(s.values()) for s in start_points_dict])
            # print('start_points_arr:', start_points_arr)
            for index, start_point in enumerate(start_points_arr):
                res = minimize(self._acqWrapper, x0=start_point, method=method,
                               bounds=self.parameter_range)
                x_best[index], f_best[index] = res.x, np.atleast_1d(res.fun)[0]
            # print('x_best[np.argmin(f_best)]:', x_best[np.argmin(f_best)])

            temp = round(x_best[np.argmin(f_best)][0], 6)
            x_best[np.argmin(f_best)][0] = temp
            # print('temp:', [temp])
            for index in self.GP.X:
                if list(index) == list(x_best[np.argmin(f_best)]):
                    count = 1
            if count == 1:
                flag = True
            else:
